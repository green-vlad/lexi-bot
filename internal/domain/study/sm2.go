...
//
// Неизвестная оценка и отложенная карточка оставляют состояние без изменений:
// планировщик — не место для проверки входных данных, этим занят сценарий,
// а молча испортить состояние хуже, чем ничего не сделать.
func (s SM2) Next(cur CardState, r Rating, now time.Time) CardState {
	if !r.IsValid() || cur.State == StateSuspended {
		return cur
	}
