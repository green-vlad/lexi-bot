...
type CardRepo interface {
	...
	// DueBefore возвращает сроки карточек, которые подойдут до момента
	// until, по возрастанию.
	//
	// Именно сроки, а не их подсчёт по дням: границы суток зависят
	// от таймзоны человека и перевода часов, и считать их лучше там, где
	// это уже умеют, — в домене. Объём ограничен окном прогноза, неделей.
	DueBefore(ctx context.Context, courseID study.CourseID, until time.Time) ([]time.Time, error)
