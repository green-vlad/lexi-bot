...
// Значения по умолчанию для нового пользователя: пять слов в день — минут
// десять занятий, а лимит повторений с запасом перекрывает нагрузку, которую
// эти пять слов создадут через месяц. Норму можно поднять в настройках;
// начинать с большей незачем — очередь повторений растёт из неё с задержкой
// в несколько дней, и человек замечает это, когда отступать поздно.
const (
	DefaultNewPerDay     = 5
...
// NextDayStart возвращает начало следующих суток пользователя: до него
// отложено слово, которое человек пропустил на знакомстве.
func (s Settings) NextDayStart(t time.Time) time.Time { return s.Timezone.NextDayStart(t) }
