...
Машина: 1 vCPU, 2 ГБ памяти, 40–50 ГБ диска, Ubuntu 24.04 LTS. Больше не нужно —
в работе приложение занимает 400–600 МБ, остальное уходит в страничный кеш, и это
ровно то, что делает Postgres быстрым при нашем объёме данных.

**Локацию выбирают не по близости к себе.** Пользователь общается с Telegram напрямую,
а наш сервер — только с Bot API; единственное плечо, на которое мы влияем, это
«сервер ↔ Telegram». Поэтому берут дата-центр рядом с тем, который обслуживает бота
(для Азии — Сингапур, для Европы — Германия или Финляндия). Проверить можно замером:

```sh
for i in $(seq 1 20); do
  curl -o /dev/null -s -w '%{time_total}\n' https://api.telegram.org/bot<ТОКЕН>/getMe
done | sort -n | awk '{a[NR]=$1} END {print "медиана", a[int(NR/2)]}'
```

Настройку операционной системы делает [`deploy/provision.sh`](../deploy/provision.sh) —
Docker с плагином compose, пользователь `deploy` в группе `docker`, `ufw` только на порт
SSH, автообновления безопасности с ночной перезагрузкой, ротация docker-логов
(`max-size: 10m`, `max-file: 3`), swap на 2 ГБ и каталог `/opt/lexi-bot`. Скрипт
идемпотентен: повторный запуск сообщает, что уже сделано, и ничего не ломает.
...
Вход по паролю скрипт выключает только тогда, когда у `deploy` уже лежит SSH-ключ:
скрипт, запирающий администратора снаружи, хуже сервера с парольным входом.

Дальше — руками, потому что дальше идут секреты:

```sh
scp docker-compose.prod.yml deploy@<адрес>:/opt/lexi-bot/
ssh deploy@<адрес>
cd /opt/lexi-bot
install -m 600 /dev/null .env      # заполнить по образцу .env.prod.example
...
```sh
docker compose -f docker-compose.prod.yml logs bot   # молчащий бот обычно ругается здесь
curl -fsS http://127.0.0.1:8080/healthz              # 200 и ok
docker ps                                            # у postgres нет проброса на хост
nmap -Pn <адрес>                                     # снаружи виден только SSH
```

Первая из них не формальность: при неверном `BOT_TOKEN` контейнер уходит в цикл
перезапусков молча, `docker ps` показывает `Restarting`, и без лога причина не видна.

Чего в compose-файле намеренно нет:

* **Порта Postgres.** Ни на `0.0.0.0`, ни на `127.0.0.1` — бот ходит в базу по внутренней
  сети Docker. Это второй рубеж после `ufw`: даже при открытом файрволе снаружи некуда
  стучаться.
* **Healthcheck у бота.** Образ distroless, в нём нет ни шелла, ни `curl`, и написать
  проверку нечем. Живость смотрят снаружи — `/healthz` с самого хоста, как это делает
  джоб деплоя.
* **Строки подключения в `.env`.** Она собирается в compose-файле из `POSTGRES_PASSWORD`:
  пароль в ней тот же самый, и держать его в двух местах — верный способ однажды
  поменять только в одном.
